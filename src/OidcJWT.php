...
class OidcJWT
{
  ...
  public static function encrypt(array $payload, string $keyAlg, string $encAlg, ?string $secret = null): string{
    ...
    if(!is_null($secret))
      $encKey = OidcKeys::genSecretKey($secret, 'enc');
    else
      $encKey = OidcKeys::get_public_key_enc();
    if($encKey->has('kid'))
  ...
  public static function NestedBuilder(array $payload, string $sigAlg, string $keyAlg, string $encAlg): string{
    ...
    $sigKey = OidcKeys::get_private_key_sign();
    $sigProtectedHeader = [
      'alg' => $sigAlg,
    ];
    if($sigKey->has('kid'))
  ...
  public static function NestedLoader(string $jwe, ?array $jwkSet = null, ?string $client_secret = null, ?array $jwkSetDec = null): array{
    ...
    if(is_null($jwkSetDec))
      $jwkSetDec = $jwkSet;
    $SigKeySet = OidcKeys::genKeySetForVerif($jwkSet, $client_secret);
    $EncKeySet = OidcKeys::genKeySetForDec($jwkSetDec, $client_secret);
  ...
  public static function decrypt(string $jwt, ?array $jwkSet = null, ?string $client_secret = null): string{
    ...
    $EncKeySet = OidcKeys::genKeySetForDec($jwkSet, $client_secret);
  ...
  public static function getJWTHeader(string $jwt): array{
    $ar = explode('.', $jwt);
    $header = json_decode(OidcUtils::base64url_decode($ar[0]), true);
    return $header;
  }
  ...
  public function signPayload(array $payload, array $hOptions = []): string{
    ...
    if(!is_null(OidcKeys::get_public_key_sign())){
      foreach($hOptions as $k){
        if(OidcKeys::get_public_key_sign()->has($k))
          $options[$k] = OidcKeys::get_public_key_sign()->get($k);
      }
    }
    $jws = $jwsBuilder
      ->create()
